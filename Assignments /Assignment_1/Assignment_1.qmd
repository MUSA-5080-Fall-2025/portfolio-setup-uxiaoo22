...
Focused on Texas counties using ACS 2022 5-year data. Variables selected: median household income, total population, and race/ethnicity (tract level). Cleaned county names by removing state suffixes for readability. Categorized margins of error (MOE) into High, Moderate, Low confidence levels for analysis clarity. Selected counties (Culberson and Dimmit) for tract-level study to highlight data reliability challenges

**Limitations:**

ACS data for small, rural counties often carries large MOEs, limiting reliability. Analysis restricted to Texas counties; findings may not generalize to other states. Reliance on 5-year estimates improves stability but masks short-term trends. MOE thresholds (<5%, 5–10%, >10%) are heuristic and may oversimplify nuanced reliability issues
